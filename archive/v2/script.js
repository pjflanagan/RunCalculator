...
var split_distances = [200, 400, 800, 1000, convert(1)];
...
var total_seconds = 0,
	...
	pacemode = false,
	distmode = false,
	unit = false; //true for kilos false for miles
...
function output() {
	...
	if (!distmode)
		factor = !pacemode
			? split_distances[split] / event_distances[race]
			: event_distances[race] / split_distances[split];
	else {
		distance = unit
			? document.getElementById('dist-num').value * 1000
			: convert(document.getElementById('dist-num').value);
