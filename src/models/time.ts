...
type DisplayTimeOut = [number, number, number, number, string];

export const makeDisplayTime = (time: number): DisplayTimeOut => {

  let hourmode = false;
  let rm10 = Math.floor(time / 60 / 10);
  if (rm10 >= 6) {
    hourmode = true;
  }


  let rm1 = Math.floor(time / 60 % 10);
  let rs10 = Math.floor(time % 60 / 10);
  let rs1 = Math.floor(time % 60 % 10);
  let dec = "." + ((time % 60 % 10 % 1 + "").substr(2) + "000").substr(0, 3);
